...
//        options.db   = parsedUrl.pathname && parsedUrl.pathname.replace(/\//g, '') || options.db;
      ...
      options.json.store =
        (args && args.jsonstore) || options.json.store;
      options.json.kv =
        (args && args.jsonkv) || options.json.kv;
      options.json.qs =
        (args && args.jsonqs) || options.json.qs;
//      options.db   = args && args.db   || options.db;
  ...
  */
  function deleteJSON(uri,callback) {
    var del = { uri: urls.json.store(uri)
              , method: "DELETE"
              };
  ...
  */
  ...
  return { "options":   { "current": options
                        , "set": setOption
                        , "get": getOption
                        }
