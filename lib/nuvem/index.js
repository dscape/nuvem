...
//      options.db   = args && args.db   || options.db;
  ...
  */ 
  function insertJSON(uri,document,callback){
    storeJSON(uri,document,"PUT",callback);
  };
  ...
  * @see storeJSON
  */ 
  function updateJSON(uri,document,callback){
    storeJSON(uri,document,"POST",callback);
  };
  ...
  * @param {uri} The document uri
  *
  * @return The JSON document that was deleted from the database
  */
  function deleteJSON(uri,callback) {
    ...
    request(get, function deleteJSONCb(e,resp,body) {
      if(e) 
        return callback(err.socketError(e, "CONNREFUSED"));
      if (resp.statusCode === 404)
        return callback(err.jsonError("Document not found", "DOCNOTFOUND"));
  ...
  * @return undefined
  */
  function storeJSON(uri,document,verb,callback) {
